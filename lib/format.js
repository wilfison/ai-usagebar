...
/** Printf-style conversion specifier: `%s`, `%d`, an optional numeric width
 * (`%2d`, `%02d`), or a literal `%%`. */
const CONVERSION = /%([0-9]*)([sd%])/gu;

/**
 * Minimal pure printf — substitutes `%s`/`%d` (with optional `%0Nd`/`%Nd` width)
 * conversions in `template` with `args`, in order; `%%` yields a literal `%`.
 *
 * Used to interpolate values into a gettext-translated format string without
 * depending on GJS's shell-injected `String.prototype.format` (which is absent
 * in the prefs process and in bare-`gjs -m` unit tests). The translatable text
 * stays a plain string literal inside `_()`, so `xgettext` still extracts it.
 * @param {string} template - format string, e.g. `_('Claude %s')`.
 * @param {...*} args - values substituted for the conversions, left to right.
 * @returns {string}
 */
